...
```mermaid
flowchart LR
    SN[ServiceNow] -->|"POST /v1/reviews (x-api-key)"| GW[API Gateway<br/>REST]
    GW --> API[API Lambda]
    API -->|Event 非同期起動| W[Worker Lambda]
    W -->|書込| DDB[(DynamoDB<br/>ジョブ)]
    W -->|出力 Excel| S3[(S3)]
    SN -.->|"poll: GET /v1/reviews/&#123;job_id&#125;"| GW
    API -.->|GetItem 読込| DDB
```

> **ServiceNow は DynamoDB を直接見ない。** ポーリングは REST API（`GET /v1/reviews/{job_id}`）を繰り返すだけで、DynamoDB を読むのは API Lambda（サーバ側）。詳細は [servicenow-integration.md #ポーリングの仕組み](servicenow-integration.md#5-ポーリングの仕組み)。
...
| C3 | RAG retrieve（fulltext）+ Bedrock 評価 + 出力スキーマ（reviewer 実装） | ✅ 実装済 |
| C4 | PII フィルタ（基本版済）・進捗（済）・サマリー（済）・エラー処理の強化 | 一部済 |
| C5 | retrieval 戦略切替（Bedrock KB / corpus2skill）対応 | — |

> **C2 の実装メモ**: Excel 入出力は [`backend/src/services/excel_io.py`](../../backend/src/services/excel_io.py)（ヘッダ別名での列特定・空行スキップ・右側への AI 列追記）。
>
> **C3 の実装メモ**: 判定は [`reviewer.py`](../../backend/src/services/reviewer.py) の `Reviewer.review()` が
> retrieve → プロンプト構築 → Bedrock → JSON パースを統合。
> - **Provider 層**: [`providers/claude_bedrock.py`](../../backend/src/services/providers/claude_bedrock.py)（Bedrock Converse, Claude Sonnet 4.6）。`get_provider(model_id)` で切替。
> - **Retrieval 層**: [`retrieval/`](../../backend/src/services/retrieval/) の `none` / `fulltext`。`get_strategy(strategy_id)` で切替。**fulltext は現状インメモリ seed**（[`reference_store.py`](../../backend/src/services/reference_store.py)）で、Phase 2 で Aurora FULLTEXT に置換。
> - **プロンプト/後処理**: [`prompt.py`](../../backend/src/services/prompt.py)（実リスク焦点・JSON 出力指示・REFERENCE_CONTEXT 注入）、[`postprocess.py`](../../backend/src/services/postprocess.py)（JSON 抽出・正規化）、[`pii_filter.py`](../../backend/src/services/pii_filter.py)（送信前マスク）。
> - LLM/パース失敗時は `needs_review` / 信頼度 low にフォールバック。
> - 見本出力: [`doc/samples/sample-application-reviewed.xlsx`](../samples/sample-application-reviewed.xlsx)。
